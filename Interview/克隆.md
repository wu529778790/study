...
```
我们可以看到，很明显虽然oldObj.c.h被克隆了，但是他还与oldObj.c.h相等，这表明他们一仍然只想同一段堆内存，这就造成了如果对newObj.c.h进行修改，也会影响到oldObj.c.h， 这就不是一个好的克隆
...
当然有一个新的api Object.assign()也可以实现浅复制，但是效果跟上面没有差别

# 深克隆

## JSON.parse方法

传说中最便捷实现深克隆的方法，JSON随想parse方法可以将JSON字符串反序列化成js对象，stringify方法可以将js对象序列化成JSON字符串，这两个方法结合起来就能产生一个便捷的深克隆
...
2. 会抛弃对象的constructor，所有的构造函数都会只想Object
3. 对象有循环引用，会报错
